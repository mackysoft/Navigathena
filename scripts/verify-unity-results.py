...
def verify(directory):
    ...
    for path in directory.rglob("*.xml"):
        ...
        assemblies.extend(root.findall(".//test-suite[@type='Assembly'][@name='MackySoft.Navigathena.Unity.Tests.dll']"))
    if len(assemblies) != 1:
    ...
    if assembly.find("./properties/property[@name='platform'][@value='PlayMode']") is None:
        raise ValueError("Navigathena adapter tests must execute in PlayMode")
    ...
    print(f"Unity tests: {total} passed")
